...
def main(save_confusion=False):
    ...
    data4 = scio.loadmat(os.getcwd() + path + 'emotion_intensity_5s' + ".mat")
    x_cont = data1['cont']
    x_eye = data2['eye']  # pupil
    # x_eye = x_eye[..., np.newaxis]
    x_phy = data3['phy']
    y = data4['emotion_intensity']
    '''
    stress1 = 0
    stress2 = 0
    stress3 = 0
    ...
    for i in range(y.shape[0]):
        if y[i] < 1:
            ...
            stress3 += 1
    '''
    emotion_intensity1 = 0
    emotion_intensity2 = 0
    emotion_intensity3 = 0
    emotion_intensity4 = 0
    emotion_intensity5 = 0
    # Divide label
    for i in range(y.shape[0]):
        if 0 < y[i] <= 2:
            y[i] = 1
            emotion_intensity1 += 1
        elif 2 < y[i] <= 4:
            y[i] = 2
            emotion_intensity2 += 1
        elif 4 < y[i] <= 5:
            y[i] = 3
            emotion_intensity3 += 1
        elif 5 < y[i] <= 7:
            y[i] = 4
            emotion_intensity4 += 1
        elif 7 < y[i] <= 9:
            y[i] = 5
            emotion_intensity5 += 1
    ...
    for k, (train_index, test_index) in enumerate(kf.split(X_train_eye1)):
        if k == 2:
            ...
            y_train = to_categorical(y_train1, 6)
            y_train = y_train[:, 1:6]
            y_test = to_categorical(y_test1, 6)
            y_test = y_test[:, 1:6]
            ...
            # es = EarlyStopping(mode='max', patience=10, verbose=1, restore_best_weights=True)

            history = model.fit(x_train, y_train, batch_size=48, epochs=50, verbose=1, validation_split=0.1,
                                shuffle=True,
                                callbacks=[es])
            _, accuracy = model.evaluate(x_test, y_test, batch_size=16, verbose=0)
            '''
            history = model.fit(x_train, y_train, batch_size=16, epochs=50, verbose=1, validation_split=0.1,
                                shuffle=True,
                                callbacks=[es])
            _, accuracy = model.evaluate(x_test, y_test, batch_size=16, verbose=0)
'''
...
def define_model():
    ...
    lstm_out12 = LSTM(64, return_sequences=False)(lstm_out11)
    ...
    lstm_out22 = LSTM(64, return_sequences=False)(lstm_out21)
    ...
    lstm_out32 = LSTM(64, return_sequences=False)(lstm_out31)
